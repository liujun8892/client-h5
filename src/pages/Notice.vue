<template>
	<div id="classDetails">
		<div v-if="postDe" class="whiteBg">
			...
			<commentList ref="commentListWoods" :user_post_id="woods_post_id" :mold="3" v-if="postDe.title"></commentList>
...
export default {
	...
	methods: {
		getWoodsNoticeDetail() {
			this.$api.getWoodsNoticeDetail({ notice_id: this.woods_post_id }).then(res => {
				console.log(res);
				if(res.code!=200){
        ...
        getShareAddress(){
			...
			console.log('url',url);
			this.$api.getShareAddress({ type: 8, relation_id: this.woods_post_id, url: url }).then(res => {
				console.log('获取分享信息');
				console.log(res)
				if (res.code == 200) {
    ...
    created(){
        if (GetQueryString('notice_id')) {
            this.woods_post_id = GetQueryString('notice_id');
